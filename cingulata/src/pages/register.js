...
export default function Register(props) {
    ...
    const [invite, setInvite] = useState(params.get("inv")||undefined);
    const [password, setPassword] = useState("")
    const [email, setEmail] = useState("")

    const doLogin = (e) => {
        e.preventDefault()
        if(!email || !password) return props.setStatus("you must have both email and password", "warning", 5)
        ...
        .catch(e => {
            if(e.status === 403) {
                if(e.message.startsWith("invalid")) return props.setStatus("invalid invite code", "error", 5)
                else return props.setStatus("invite only.", "error", 5)
            }
            props.setStatus("Registration failed. Do you already have an account?", "error", 5)
            console.warn(e)
        })
    }
            ...
            <form onSubmit={(e) => doLogin(e)}>
                    <div className="group">
                        ...
                        <div className="row">
                            ...
                            <input onChange={ e => setEmail(e.target.value) } value={email} name="email" placeholder="email" type="email"></input>
                        </div>
                    </div>
                    <div className="group">
                        ...
                        <div className="row">
                            ...
                            <input onChange={ e => setPassword(e.target.value) } value={password} minLength="5" name="password" placeholder="password" type="password"></input>
                        </div>
                    </div>
